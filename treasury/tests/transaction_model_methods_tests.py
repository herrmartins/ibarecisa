...
class FakeStorage(Storage):
    ...
    def path(self, name):
        normalized_path = os.path.normpath(os.path.join(self.base_location, name))
...
class TransactionModelMethodsTests(TestCase):
    ...
    def test_delete_transaction(self):
        ...
        self.assertIsNone(
            still_exists, "Transaction should not exist in the database anymore."
        )
    ...
    def test_update_transaction_changes_file(self):
        ...
        self.assertFalse(
            transaction.acquittance_doc.storage.exists(old_image_path),
            "Old file should have been deleted but still exists.",
        )

    def test_update_transaction_no_file_change(self):
        transaction = self.create_transaction()
        with patch.object(
            transaction.acquittance_doc, "delete", MagicMock(name="delete")
        ) as mock_delete:
    ...
    def test_storage_delete_directly(self):
        ...
        self.assertFalse(
            file_name in self.storage.files,
